...
registered and deregistered at will. See the :py:class::`EventManager` class
...
use the same :py:class::`Event` subclass, which is instantiated and passed in
to the events on event call, also passing in any data required.
...
A :py:class::`CAPEvent` is passed in.
...
A :py:class::`LineEvent` is passed to each callback, containing the line that
triggered the event.
...
.. note::
   This hook requires the :py:class::`CapNegotiate` extension.
...
A :py:class::`HookEvent` is passed in.

commands_ctcp
^^^^^^^^^^^^^

.. note::
   This hook requires the :py:class::`CTCP` extension.

This hook is used by the :py:class::`CTCP` extension to call CTCP events.
VERSION and PING events are implemented by default.

A :py:class::`CTCPEvent` is passed in, which passes in the
:py:class::`CTCPMessage` from the event.
...
This hook is used by the :py:class::`CTCP` extension to call NCTCP events;
that is, the reply to CTCP queries. No events are registered by default.
